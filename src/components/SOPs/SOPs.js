...
class SOPs extends Component {
  ...
  componentDidMount() {
    ...
    var arr = []
    database.ref('SOPs/').on('value', function (snap) {
      arr = []
  ...
  test(e) {
    ...
    var fileSend = storage.ref('SOPs/' + fileUpload.name);
    ...
    task.on("state_changed", function (snap) {
      ...
      if (percentage === 100) {
        ...
        setTimeout(function() {storageRef.child('SOPs/' + fileUpload.name).getDownloadURL().then(function (url) {
          
          var firebaseRef = database.ref('SOPs/');
          var data = {
            url: url,
            fileTitle: fileTitle,
          ...
          firebaseRef.push(data).then(function () {
            message.success(fileTitle + ' has been added');
          }).then(() => {
  ...
  open(url, title, fileTitle) {
    ...
    database.ref('SOPs').orderByChild('url').equalTo(url).once("value", function (snap) {
      snap.forEach(function (data) {
        key = data.key
      });
      database.ref('SOPs/' + key).once('value', function (snap) {
        snap.forEach(function (data) {
  ...
  delete() {
    var desertRef = storageRef.child('SOPs/' + this.state.title);
    ...
    database.ref('SOPs/' + this.state.key).remove();
    ...
    return (
      <div>
        <Col xs={{ span: 20, offset: 2 }} sm={{ span: 12, offset: 6 }} style={{ marginTop: '5em', marginBottom: '5em' }} >
          <h2>Standard Operating Procedures/Guidelines</h2>
          <br />


        {this.state.loadedFile ?
            <div style={{ marginBottom: '15px', marginTop: '5px' }} >
              <Input id='fileTitle' placeholder='Enter SOP Title' onChange={this.onChangeTitle.bind(this)} />
          ...
          <div className="image-upload">
            <label htmlFor="file-input">
              <a>{this.state.uploadLabel ? 'Upload SOPs  ' : null}</a>
            </label>

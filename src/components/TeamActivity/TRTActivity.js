...
class TRTActivity extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentDidMount() {
    ...
    if (this.state.users) {
      this.state.users.forEach(function (name) {
        if(name.trt === 'Yes') {team.push(name)}
      })
    }
    ...
    this.state.rosters.forEach(function (datum) {
      ...
      if (datum.type === "Monthly Training - TRT") {
        trainings.push({ date: datum.date, type: datum.type, description: datum.description, members: datum.members, uid: datum.uid })
          data.push(datum)
      } else if (datum.type === "Truck Check") {
        truckCheck.push({ date: datum.date, title: datum.runOrTitle, members: datum.members })
          data.push(datum)
      } else if (datum.type === "Confined Space" ||
        datum.type === "Rope Rescue" ||
        datum.type === "Trench Rescue" ||
        datum.type === "Building Collapse" ||
        datum.type === "Vehicle Extrication" ||
        datum.type === "Grain Bin Rescue" ||
        datum.type === "Other"
      ) {
  ...
  render() {
      ...
      trainingAttended = [];
        this.state.trainings.map((item) => (
          item.members.map((newItem) => (
            name === newItem ? trainingAttended.push({ date: item.date, type: item.type, description: item.description, members: item.members }) : null
          ))
        ))
    ...
    var incidentsTotal = (name) => {
      incidentAttended = [];
        this.state.incidents.map((item) => (
          item.members.map((newItem) => (
            name === newItem ? incidentAttended.push({ date: item.date, title: item.title }) : ''
          ))
        ))
    ...
    var truckCheckTotal = (name) => {
      ...
      this.state.truckCheck.map((item) => (
        item.members.map((newItem) => (
          name === newItem ? truckCheckAttended.push({ date: item.date, title: item.title }) : ''
        ))
      ))
